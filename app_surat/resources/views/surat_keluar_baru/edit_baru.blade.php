...
<div class="hk-pg-wrapper">
    ...
    <nav class="hk-breadcrumb" aria-label="breadcrumb">
        <ol class="breadcrumb breadcrumb-light bg-transparent">
            ...
            <li class="breadcrumb-item active" aria-current="page">Edit Surat Keluar (BARU)</li>
        </ol>
    </nav>
    <!-- /Breadcrumb -->
    ...
    <div class="container">
        ...
        <div class="hk-pg-header">
            <h4 class="hk-pg-title"><span class="pg-title-icon"><span class="feather-icon"><i data-feather="align-left"></i></span></span>Edit Surat Keluar (BARU)</h4>
        </div>
        <!-- /Title -->
                ...
                <section class="hk-sec-wrapper">
                    <div class="row">
                        ...
                        <div class="col-sm d-flex justify-content-end align-items-start ">
                            <a class="btn btn-indigo btn-wth-icon btn-rounded icon-left" href="{{ url('/surat_keluar_baru')}}"><span class="btn-text">Kembali</span> <span class="icon-label"><span class="feather-icon"><i data-feather="arrow-left-circle"></i></span> </span></a>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-sm">
                            <form action="{{ url('/surat_keluar_baru/'.$surat->id.'/update')}}" method="POST" class="needs-validation" enctype="multipart/form-data" novalidate>
                                @csrf
                                ...
                                <div class="row">
                                    <div class="col-md-6 form-group">
                                        ...
                                        <input type="text" class="form-control" id="penandatangan_surat" name="penandatangan_surat" placeholder="Penandatangan Surat" 
                                        value="{{ $surat->penandatangan_surat }}" readonly>
                                    </div>

                                    <div class="col-md-6 form-group">
                                        {{-- <label for="jabatan_penandatangan_surat">Jabatan Penandatangan</label>
                                        <select class="custom-select" id="jabatan_penandatangan_surat" name="jabatan_penandatangan_surat" required>
                                            <option selected>--Pilih Jabatan--</option>
                                            <option value="KPTA" {{ $surat->jabatan_penandatangan_surat=='KPTA' ? 'selected':'' }}>Ketua (KPTA)</option>
                                            <option value="WKPTA" {{ $surat->jabatan_penandatangan_surat=='WKPTA' ? 'selected':'' }}>Wakil Ketua (WKPTA)</option>
                                            <option value="PAN" {{ $surat->jabatan_penandatangan_surat=='PAN' ? 'selected':'' }}>Panitera (PAN)</option>
                                            <option value="SEK" {{ $surat->jabatan_penandatangan_surat=='SEK' ? 'selected':'' }}>Sekretaris (SEK)</option>
                                        </select>
                                ...
                                <div class="row">
                                    <div class="col-md-6 form-group">
                                        <label for="kode_penetapan">Kode Penetapan</label>
                                        <select class="custom-select" id="kode_penetapan" name="kode_penetapan">
                                            <option value = "" selected>--Tidak Ada Kode Penetapan--</option>
                                            <option value="SK" {{ $surat->kode_penetapan=='SK' ? 'selected':'' }}>Surat Keputusan (SK)</option>
                                            <option value="SP" {{ $surat->kode_penetapan=='SP' ? 'selected':'' }}>Surat Perintah (SP)</option>
                                            <option value="ST" {{ $surat->kode_penetapan=='ST' ? 'selected':'' }}>Surat Tugas (ST)</option>
                                            <option value="SKET" {{ $surat->kode_penetapan=='SKET' ? 'selected':'' }}>Surat Keterangan (SKET)</option>
                                            <option value="PENG" {{ $surat->kode_penetapan=='PENG' ? 'selected':'' }}>Pengumuman (PENG)</option>
                                            <option value="UND" {{ $surat->kode_penetapan=='UND' ? 'selected':'' }}>Surat Undangan (UND)</option>
                                        </select>
                                        <div class="invalid-feedback">
                                            Perbaiki inputan
                                        </div>
        ...
        <footer class="footer">
            <div class="row">
                <div class="col-md-6 col-sm-12">
                    <p>Hak Cipta<a href="http://www.pta-bandung.go.id" class="text-dark" target="_blank">PTA Bandung</a> Â© 2022</p>
                </div>
                <div class="col-md-6 col-sm-12">
