...
class SuratkeluarbaruModel extends Model
{
    use HasFactory;
    protected $table = 't_surat_keluar_baru';
    protected $fillable = [
        'id',
        'no_agenda',
        'no_urut',
    ...
    public function get_data($tahun) {
        $data = DB::table('t_surat_keluar_baru')->select('t_surat_keluar_baru.*')
    ...
    public function get_data_ajax($request, $origin=null) {
        ...
        $sql = "SELECT * FROM t_surat_keluar_baru
    ...
    public function get_statistik($bulan, $tahun) {
        ...
        $sql = "SELECT k.id, k.kode, k.nama, k.uraian, count(k.id) jumlah
                FROM t_surat_keluar_baru sk JOIN ref_klasifikasi_baru k ON sk.id_klasifikasi=k.id
    ...
    public function get_jumlah_surat($tahun) {
        $sql = "SELECT MONTH(tanggal_surat) bulan, COUNT(id) jumlah FROM t_surat_keluar_baru
    ...
    public function get_statistik_parent($bulan, $tahun) {
        ...
        $sql = "SELECT parent, SUM(jumlah) jumlah FROM (
                    ...
                    FROM t_surat_keluar_baru sk JOIN ref_klasifikasi_baru k ON sk.id_klasifikasi=k.id
                    JOIN ref_klasifikasi_baru p ON k.parent_id=p.id
